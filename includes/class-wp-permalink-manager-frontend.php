...
class WP_Permalink_Frontend {
	...
	public function wpml_permalink_filter( $permalink, $language_code ) {
		$wp_permalink_manager   = $permalink;
		$trailing_permalink = trailingslashit( home_url() ) . $wp_permalink_manager;
		if ( $language_code ) {
			...
			if ( 0 === strpos( $wpml_href, '//' ) ) {
				if ( 0 !== strpos( $wpml_href, '//' . $language_code . '/' ) ) {
					$permalink = $site_url . '/' . $language_code . '/' . $wp_permalink_manager;
				}
			}
		} else {
		...
		if ( ! $posts ) {
			...
			$posts = $wpdb->get_results(
				$wpdb->prepare(
					...
					" WHERE pm.meta_key = 'wp_permalink_manager' " .
			...
			if ( ! $posts && '__true' === $remove_like_query ) {
				...
				$posts = $wpdb->get_results(
					$wpdb->prepare(
						...
						" meta_key = 'wp_permalink_manager' AND meta_value != '' AND " .
	...
	public function parse_request( $query ) {
		...
		$ignore = apply_filters( 'wp_permalink_manager_exclude_permalink', $request );
		...
		if ( null !== $original_url ) {
			...
			$path_info              = apply_filters(
				'wp_permalink_manager_path_info',
				'__false'
			);
	...
	public function make_redirect() {
		...
		if ( ! isset( $posts[0]->ID ) || ! isset( $posts[0]->meta_value )
			...
			if ( ( is_single() || is_page() ) && ! empty( $wp_query->post ) ) {
				...
				$wp_permalink_manager   = $this->term_permalink( $the_term->term_id );
				$original_permalink = $this->original_term_link( $the_term->term_id );
			}
		} else {
			$wp_permalink_manager = $posts[0]->meta_value;
			if ( 'page' === $posts[0]->post_type ) {
		...
		$custom_length = strlen( $wp_permalink_manager );
		if ( $wp_permalink_manager
			&& (
				substr( $request, 0, $custom_length ) !== $wp_permalink_manager
				|| $request === $wp_permalink_manager . '/'
			)
		) {
			// Request doesn't match permalink - redirect.
			$url             = $wp_permalink_manager;
	...
	public function custom_post_link( $permalink, $post ) {
		$wp_permalink_manager = get_post_meta( $post->ID, 'wp_permalink_manager', true );
		if ( $wp_permalink_manager ) {
			$post_type = 'post';
	...
	public function custom_page_link( $permalink, $page ) {
		$wp_permalink_manager = get_post_meta( $page, 'wp_permalink_manager', true );
		if ( $wp_permalink_manager ) {
			$language_code = apply_filters(
				'wpml_element_language_code',
				null,
	...
	public function custom_term_link( $permalink, $term ) {
		if ( isset( $term ) ) {
			if ( isset( $term->term_id ) ) {
				$wp_permalink_manager = $this->term_permalink( $term->term_id );
			}
