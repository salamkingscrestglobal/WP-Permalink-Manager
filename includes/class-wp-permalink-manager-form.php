...
class WP_Permalink_Manager_Form {
	...
	private function exclude_permalink_manager( $post ) {
		...
		$exclude_post_types = apply_filters(
			'wp_permalink_manager_exclude_post_type',
			$post->post_type
		);
	...
	public function protect_meta( $protected, $meta_key ) {
		if ( 'wp_permalink_manager' === $meta_key ) {
			$protected = true;
		}
	...
	public function save_post( $post_id, $post ) {
		...
		if ( ! empty( $_REQUEST['wp_permalink_manager'] )
			&& $_REQUEST['wp_permalink_manager'] !== $original_link
		) {
			...
			update_post_meta( $post_id, 'wp_permalink_manager', $permalink );
		}
	}
	...
	private function get_permalink_html( $post, $meta_box = false ) {
		...
		if ( 'page' === $post->post_type ) {
			...
			$view_post          = __( 'View Page', 'wp-permalink-manager' );
		} else {
			$post_type_name   = '';
			...
			$view_post          = __( 'View', 'wp-permalink-manager' ) . $post_type_name;
		}
	...
	private function get_permalink_form( $permalink, $original, $id,
		...
		'<input value="' . $encoded_permalink . '" type="hidden" name="wp_permalink_manager" id="wp_permalink_manager" />';
		...
		'<span id="editable-post-name" title="Click to edit this part of the permalink">' .
			...
			'<input type="text" id="wp-permalink-manager-post-slug" class="text" value="' . $post_slug . '" style="' . $field_style . '" />' .
		'</span>';
		// phpcs:enable WordPress.Security.EscapeOutput.OutputNotEscaped
		...
		if ( $render_containers ) {
			echo '<br />' .
			'<small>' .
				esc_html__( 'Leave blank to disable', 'wp-permalink-manager' ) .
			'</small>' .
			'</td>' .
			'</tr>' .
			'</table>';
	...
	public function save_term( $term_id ) {
		...
		if ( isset( $term ) && isset( $term->taxonomy ) ) {
			...
			if ( 'category' === $taxonomy_name || 'post_tag' === $taxonomy_name ) {
				...
				$new_permalink = ltrim(
					// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
					stripcslashes( $_REQUEST['wp_permalink_manager'] ),
					'/'
				);
	...
	public function refresh_meta_form( $data ) {
		if ( isset( $data['id'] ) && is_numeric( $data['id'] ) ) {
			...
			$all_permalinks['wp_permalink_manager'] = get_post_meta(
				$data['id'],
				'wp_permalink_manager',
				true
			);

			if ( ! $all_permalinks['wp_permalink_manager'] ) {
				if ( 'draft' === $post->post_status ) {
					$view_post_link = '?';
					if ( 'page' === $post->post_type ) {
						$view_post_link .= 'page_id';
				...
				$all_permalinks['wp_permalink_manager'] = htmlspecialchars(
					urldecode(
						$all_permalinks['wp_permalink_manager']
					)
				);
			}
